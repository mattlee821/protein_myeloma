...
## UKB proteins
data <- fread("analysis/002_outcomes/instruments-cancer_outcome-proteinsUKB.txt", sep = " ", col.names = c(
  "chr.outcome",
  "pos.outcome",
  "ID",
  "other_allele.outcome",
...
data <- format_data(data, type = "outcome", snps = NULL,  header = TRUE,  
...
## combine and format
data_outcome <- bind_rows(data1, data2)
...
# steiger ====
## data_outcome1 - ieu
data <- subset(data_harmonise, exposure == "ieu-b-4957")
...
data <- subset(data_harmonise, exposure == "myeloma_meta-analysis")
data$r.exposure <- get_r_from_lor(lor = data$beta.exposure, af = data$eaf.exposure, 
...
data <- subset(data_harmonise, exposure == "myeloma_finngen")
data$r.exposure <- get_r_from_lor(lor = data$beta.exposure, af = data$eaf.exposure, 
...
## data_outcome4 - UKB
data <- subset(data_harmonise, exposure == "myeloma_UKB")
data$r.exposure <- get_r_from_lor(lor = data$beta.exposure, af = data$eaf.exposure, 
