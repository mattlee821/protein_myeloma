...
write.table(exposure, "analysis/003_colocalization/results/ieu-b-4957/exposure_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

# outcome data ====
outcome <- fread("analysis/003_colocalization/outcome_window_ieu-b-4957.txt")
## transform beta from BLOT LMM scale
outcome$u <- 601/372617
outcome$beta.outcome <-  outcome$beta.outcome/(outcome$u * (1- outcome$u))
outcome$se.outcome <-  outcome$se.outcome/(outcome$u * (1- outcome$u)) 
write.table(outcome, "analysis/003_colocalization/results/ieu-b-4957/outcome_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
write.table(harmonise_data, "analysis/003_colocalization/results/ieu-b-4957/harmonise_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
for (i in 1:length(harmonise_data_list)){
  ...
  pdf(paste0("analysis/003_colocalization/results/ieu-b-4957/figures/", label, ".pdf"), 
      height = 10, width = 10)
  ...
  # save ====
  saveRDS(coloc_results, paste0("analysis/003_colocalization/results/ieu-b-4957/", label, ".RData"))
...
write.table(table_master, "analysis/003_colocalization/results/ieu-b-4957/001_coloc_results.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
