...
# exposure data ====
LIST_FILES_UKB <- c(list.files(path = "/data/GWAS_data/work/UKB_PPP/cis-snps/combined/125k/", pattern = "gz", all.files = T, full.names = T, recursive = T),
                    list.files(path = "/data/GWAS_data/work/UKB_PPP/cis-snps/european/125k/", pattern = "gz", all.files = T, full.names = T, recursive = T))
LIST_FILES_DECODE <- c(list.files(path = "/data/GWAS_data/work/ferkingstad_2021_PMID34857953/cis-snps/window/125k/", pattern = "gz", all.files = T, full.names = T, recursive = T))
...
write.table(data_exposure, "analysis/005_colocalization/data_exposure/125k/data_exposure.txt",
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
snps <- unique(data_exposure$SNP)
write.table(snps, "analysis/005_colocalization/data_exposure/125k/snplist-exposure.txt",
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
