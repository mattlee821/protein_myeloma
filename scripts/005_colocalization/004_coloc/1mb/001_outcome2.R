...
# data ====
data_harmonise <- fread("analysis/005_colocalization/data_harmonise/1mb/data_harmonise-outcome2.txt")
...
LIST_FILES <- list.files(path = "analysis/005_colocalization/data_results/1mb/outcome2/", pattern = ".RData", all.files = T, full.names = F)
...
for (i in 1:length(list_data)) {
  ...
  tryCatch({
    ...
    if (dim(ld)[1] == 0) {
      ...
      # coloc sensitivity 
      pdf(paste0("analysis/005_colocalization/data_results/1mb/outcome2/figures/", label, ".pdf"), 
          height = 10, width = 10)
      coloc_sensitivity <- my_sensitivity(coloc_results, "H4 > 0.8", 
                                          trait1_title = label, trait2_title = outcome)
      dev.off()
      # save
      saveRDS(coloc_results, paste0("analysis/005_colocalization/data_results/1mb/outcome2/", label, ".RData"))
...
# write
write.table(table_master, "analysis/005_colocalization/data_results/1mb/outcome2/001_coloc-results.txt",
